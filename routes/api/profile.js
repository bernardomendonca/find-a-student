...
// @route       GET request to api/profile/students
// @description Get all students profiles
// @access      Public
router.get("/students", (req, res) => {
  const errors = {};

  Profile.find({ status: "Student" })
    .populate("user", ["name", "avatar"])
    .then(profiles => {
      if (!profiles) {
...
// @route       GET request to api/profile/founders
// @description Get all founders profiles
// @access      Public
router.get("/founders", (req, res) => {
  const errors = {};

  Profile.find({ status: "Founder" })
    .populate("user", ["name", "avatar"])
    .then(profiles => {
      if (!profiles) {
        errors.noprofile = "There are no profiles";
...
router.get("/handle/:handle", (req, res) => {
  ...
  Profile.findOne({ handle: req.params.handle }) //.params grabs from the URL
    .populate("user", ["name", "avatar"])
    .then(profile => {
      if (!profile) {
        errors.noprofile = "There is no profile for this user";
...
router.get("/user/:user_id", (req, res) => {
  ...
  Profile.findOne({ user: req.params.user_id }) //.params grabs from the URL
    .populate("user", ["name", "avatar"])
    .then(profile => {
      if (!profile) {
        errors.noprofile = "There is no profile for this user";
